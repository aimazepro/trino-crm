...
export interface ColumnItem {
  id: string;
  label: string;
  category: "Negocio" | "Contato" | "Empresa";
  tag: string;
}

export const ALL_COLUMNS: ColumnItem[] = [
  // NEGOCIO fields
  { id: "title", label: "Título", category: "Negocio", tag: "Negocio" },
  { id: "value", label: "Valor", category: "Negocio", tag: "Negocio" },
  { id: "stage", label: "Etapa", category: "Negocio", tag: "Negocio" },
  { id: "pipeline", label: "Pipeline", category: "Negocio", tag: "Negocio" },
  { id: "company", label: "Empresa", category: "Negocio", tag: "Negocio" },
  { id: "contact", label: "Contato", category: "Negocio", tag: "Negocio" },
  { id: "owner", label: "Proprietário", category: "Negocio", tag: "Negocio" },
  { id: "status", label: "Status", category: "Negocio", tag: "Negocio" },
  { id: "createdAt", label: "Criado em", category: "Negocio", tag: "Negocio" },
  { id: "expectedCloseDate", label: "Data prevista", category: "Negocio", tag: "Negocio" },
  { id: "labels", label: "Etiquetas", category: "Negocio", tag: "Negocio" },
  { id: "probability", label: "Probabilidade", category: "Negocio", tag: "Negocio" },
  
  // CONTATO fields
  { id: "contactRole", label: "Cargo", category: "Contato", tag: "Contato" },
  { id: "contactEmail", label: "Email", category: "Contato", tag: "Contato" },
  { id: "contactPhone", label: "Telefone", category: "Contato", tag: "Contato" },
  
  // EMPRESA fields
  { id: "companyCity", label: "Cidade", category: "Empresa", tag: "Empresa" },
  { id: "companyCnpj", label: "CNPJ", category: "Empresa", tag: "Empresa" },
  { id: "companyState", label: "Estado", category: "Empresa", tag: "Empresa" },
  { id: "companySize", label: "Porte", category: "Empresa", tag: "Empresa" },
  { id: "companySegment", label: "Segmento", category: "Empresa", tag: "Empresa" },
  { id: "companyWebsite", label: "Website", category: "Empresa", tag: "Empresa" },
];

export const DEFAULT_COLUMNS = ["title", "value", "stage", "pipeline", "company", "contact", "owner", "status"];
...
export function CustomizeColumnsModal({ initialColumns, onClose, onSave }: CustomizeColumnsModalProps) {
  ...
  const handleUncheck = (columnId: string) => {
    if (columnId === "title") return; // cannot hide Title
  ...
  const invisibleByCategory = (cat: "Negocio" | "Contato" | "Empresa") => 
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-4">
                ...
                {(provided) => (
                  ...
                  >
                    {filteredVisible.map((item, index) => {
                      ...
                      return (
                        <Draggable key={item.id} draggableId={item.id} index={index} isDragDisabled={isTitle}>
                            ...
                            >
                              ...
                              <div 
                                ...
                                className={cn(
                                  ...
                                  isTitle && "opacity-0 pointer-events-none"
                                )}
                              >
                              ...
                              <div className="flex shrink-0 items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                  type="button"
                                  disabled={index <= 1} // Title is 0, so cannot move to 0
          ...
          {/* NEGOCIO category */}
          {invisibleByCategory("Negocio").length > 0 && (
            <div>
              <p className="text-xs font-semibold tracking-widest text-zinc-400 uppercase mb-2">Negocio</p>
              <ul className="space-y-0.5">
                {invisibleByCategory("Negocio").map(item => (
          ...
          {/* CONTATO category */}
          {invisibleByCategory("Contato").length > 0 && (
            <div>
              <p className="text-xs font-semibold tracking-widest text-zinc-400 uppercase mb-2">Contato</p>
              <ul className="space-y-0.5">
                {invisibleByCategory("Contato").map(item => (
                  <li key={item.id} className="flex items-center gap-2 rounded-md px-2 py-1.5 hover:bg-zinc-50 transition-colors">
                    <div className="h-3.5 w-3.5 shrink-0" />
                    <button 
                      type="button"
          ...
          {/* EMPRESA category */}
          {invisibleByCategory("Empresa").length > 0 && (
            <div>
              <p className="text-xs font-semibold tracking-widest text-zinc-400 uppercase mb-2">Empresa</p>
              <ul className="space-y-0.5">
                {invisibleByCategory("Empresa").map(item => (
                  <li key={item.id} className="flex items-center gap-2 rounded-md px-2 py-1.5 hover:bg-zinc-50 transition-colors">
                    <div className="h-3.5 w-3.5 shrink-0" />
                    <button 
                      type="button"
